...
// };
     ...
     * @param contactData - The contact form data (name, email, subject, message).
    ...
    export async function submitContactForm(contactData: {
      ...
      try {
        const url = `/api/email/contact`; // Use local API endpoint
    ...
    /**
     * Submits a custom order request with optional image uploads.
     * @param formData - FormData containing the custom order details and images.
     * @returns A promise that resolves to the response message.
     */
    export async function submitCustomOrder(formData: FormData): Promise<{ message: string }> {
      try {
        const url = `/api/email/custom-order`; // Use local API endpoint
        console.log("Submitting custom order to:", url);
        const res = await fetch(url, {
          method: 'POST',
        ...
        console.error("Error submitting custom order:", error.message);
        throw error;
      }
    }
